...
struct DeliveryCharge {
    // *** リファクタリング ***
    // マジックナンバーを定数化
    private static let chargeFreeThreshold = 2000
    private static let payCharge = 500
    private static let payFree = 0

    let amount: Int
    
    init(shoppingCart: ShoppingCart) {
        // *** リファクタリング ***
        if shoppingCart.totalPrice < DeliveryCharge.chargeFreeThreshold {
            amount = DeliveryCharge.payCharge
        } else {
            amount = DeliveryCharge.payFree
        }
    }
}
